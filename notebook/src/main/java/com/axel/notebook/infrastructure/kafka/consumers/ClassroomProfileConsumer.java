...
import com.axel.notebook.application.services.consumers.IClassroomProfileConsumer;
...
@Service
public class ClassroomProfileConsumer implements IClassroomProfileConsumer {
    ...
    @Bean
    public KafkaAdmin.NewTopics createClassroomProfileTopics() {
        return new KafkaAdmin.NewTopics(
                new NewTopic("classroomProfiles", 1, (short) 1)
        );
    }
    ...
    //Listen to "response-data-classroomProfile", only one instance from user is processed
    @KafkaListener(topics = "response-data-classroomProfile", groupId = "classRoom-group-consumer")
