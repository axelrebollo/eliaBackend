...
import com.axel.user.infrastructure.kafka.producers.CourseProfileProducer;
...
@Service
public class CourseTokenConsumer {
    //Dependency injection
    private final CourseProfileProducer courseProfileProducer;
    private final UserRepositoryImpl userRepository;
    private final JWTRepositoryImpl jwtRepository;
    private final JpaProfileRepository jpaProfileRepository;

    @Bean
    public KafkaAdmin.NewTopics createCourseTopic() {
        return new KafkaAdmin.NewTopics(
          new NewTopic("petition-idProfile-course", 1, (short)1)
        );
    }

    //Constructor
    public CourseTokenConsumer(CourseProfileProducer courseProfileProducer,
                               UserRepositoryImpl userRepository,
                               JWTRepositoryImpl jwtRepository,
                               JpaProfileRepository jpaProfileRepository) {
        this.courseProfileProducer = courseProfileProducer;
    ...
    @KafkaListener(topics = "petition-idProfile-course", groupId = "course-group")
    public void processToken(ConsumerRecord<String, String> record) {
        ...
        try{
            ...
            courseProfileProducer.sendProfileId(token, idProfile, correlationId);
        }
        catch(Exception e){
            courseProfileProducer.sendError(correlationId, "Token inválido");
        }
    }
}
